...
static LRESULT CALLBACK fenster_wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN:
	case WM_LBUTTONUP:
		f->inp.mouse = (msg == WM_LBUTTONDOWN);
		break;
	case WM_MOUSEMOVE:
		f->inp.y = HIWORD(lParam), f->inp.x = LOWORD(lParam);
		break;
	case WM_KEYDOWN:
	case WM_KEYUP: {
		f->inp.keys[FENSTER_KEYCODES[HIWORD(lParam) & 0x1ff]] = !((lParam >> 31) & 1);
		f->inp.mod = ((GetKeyState(VK_CONTROL) & 0x8000) >> 15) |
...
FENSTER_API int fenster_open(struct fenster *f) {
	...
	RegisterClassEx(&wc);
	f->hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, title, title,
		//WS_OVERLAPPEDWINDOW, 
...
FENSTER_API int fenster_loop(struct fenster *f) {
	...
	while (XPending(f->dpy)) {
		...
		case ButtonPress:
		case ButtonRelease:
			f->inp.mouse = (ev.type == ButtonPress);
			break;
		case MotionNotify:
			f->inp.x = ev.xmotion.x, f->inp.y = ev.xmotion.y;
			break;
		case KeyPress:
		case KeyRelease: {
			int m = ev.xkey.state;
			...
			for (unsigned int i = 0; i < 124; i += 2) {
				if (FENSTER_KEYCODES[i] == k) {
					f->inp.keys[FENSTER_KEYCODES[i + 1]] = (ev.type == KeyPress);
					break;
				}
			}
			f->inp.mod = (!!(m & ControlMask)) | (!!(m & ShiftMask) << 1) |
...
public:
	Fenster(const int _w, const int _h, const char* _title) {
		this->f.title = _title;
		this->f.width = _w;
		this->f.height = _h;
		this->f.buf = new uint32_t[_w * _h];
	...
	bool key(int c) { return c >= 0 && c < 128 ? (this->f.inp.keys[c] > 0) : false; }
	int x() { return this->f.inp.x; }
	int y() { return this->f.inp.y; }
	int mouse() { return this->f.inp.mouse; }
	int mod() { return this->f.inp.mod; }
};
#endif /* __cplusplus */
